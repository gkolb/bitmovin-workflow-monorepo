...
<script lang="ts">
    ...
    async function uploadFiles() {
        ...
        const { url, key } = await fetch(`http://localhost:3000/presign?filename=${encodeURIComponent(file.name)}`).then(res => res.json());
...
<div class="w-full min-w-[240px]">
    <Card.Root>
        ...
        <Card.Footer class="flex-col gap-2">
            <Button type="submit" onclick={uploadFiles} disabled={!files} class="w-full">Upload</Button>
        </Card.Footer>
    </Card.Root>
</div>
